...
class DashboardService {
  async getSummary() {
    const response =
      await api.get<Response<DashboardSummary>>("/dashboard/summary");
    return response.data;
  }

  async getUserStats(params: DashboardParams) {
    const response = await api.get<Response<UserStats>>(
  ...
  async getAdminStats(params: DashboardParams) {
    const response = await api.get<Response<AdminStats>>(
