...
const EditCommentEditor = ({ initialState, onChange }: EditCommentEditorProps) => {
  const initialConfig = {
...
const EditorInner = ({ onChange }: { onChange: (value: { raw: string; html: string }) => void }) => {
    ...
    return (
      <>
        <ToolbarPlugin />
        <RichTextPlugin
          contentEditable={<ContentEditable className="min-h-[80px] border rounded-md p-2 text-sm" />}
          placeholder={<div className="text-sm text-muted-foreground px-2 py-1">Edit your comment...</div>}
